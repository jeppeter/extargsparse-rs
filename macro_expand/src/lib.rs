...
fn _macro_ext_get_environ_var(envname :&str) -> String {
...
fn _macro_ext_proc_log_init(prefix :&str) -> i32 {
		...
		getv = _macro_ext_get_environ_var(&key);
		if getv.len() > 0 {
			msgfmt = format!("{}",getv);
		}
	    ...
	    wfile = _macro_ext_get_environ_var(&key);
	    if wfile.len() > 0 {
...
lazy_static! {
	static ref _MACRO_EXT_LOG_LEVEL : i32 = {
		_macro_ext_proc_log_init("EXTARGS_MACRO")
	};
}
