...
use extargsparse_worker::const_value::{ENV_COMMAND_JSON_SET, ENVIRONMENT_SET, ENV_SUB_COMMAND_JSON_SET};
...
fn main() -> Result<(),Box<dyn Error>> {
    ...
    let depjsoncon = r#"{"list":["depjson1","depjson2"]}"#.to_string();
    ...
    let prioritys = vec![ENV_COMMAND_JSON_SET, ENVIRONMENT_SET, ENV_SUB_COMMAND_JSON_SET];
    let parser :ExtArgsParser = ExtArgsParser::new(Some(options.clone()),Some(prioritys.clone()))?;
